...
def get_community():
    ...
    return render_template(
        ...
        iframe=get_map(gdf, water_gdf, forest_gdf, building_gdf),
        wiki=get_wiki(selected_community[0]),
    )


# Oliver's Part
# Querry to selects unique values from column amenities
def refresh_amenity():
    conn = get_db_connection()
    cur = conn.cursor()
...
# Refresh with the selected amenities
...
# Querry for the database with the selected amenities to show the results of it
...
# Selection of predifined queries for the dropdown field
